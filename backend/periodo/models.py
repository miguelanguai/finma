...
class Periodo(models.Model):
    ...
    ingreso_fijo (float): valor de ingreso fijo del periodo. Ejemplo: 1,450.37
    ingreso_variable (float): valor de ingreso variable del periodo. Ejemplo: 1,700.00
    ...
    ingreso_fijo = models.DecimalField(max_digits=7, decimal_places=2, null=True)
    ingreso_estimado = models.DecimalField(max_digits=7, decimal_places=2, null=True)

    def __str__(self):
        return f"""
        periodo {self.nombre} en fecha {self.fecha}. Fijo de {self.ingreso_fijo}. Estimado de {self.ingreso_estimado}
        """
